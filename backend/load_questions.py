"""
Скрипт для загрузки базы вопросов из JSON/CSV файла
"""
import sys
import json
import csv
...
def get_or_create_group(db: Session, group_name: str = "Тестовая группа") -> models.Group:
    ...
    if not group:
        ...
        if not teacher:
            print("Преподаватель не найден! Запустите init_db.py")
            return None
...
def load_custom_file():
    ...
    parser.add_argument('--group', '-g', type=str, default="Тестовая группа", help='Название группы')
