...
Создаёт аккаунт преподавателя: `mr.dyadichev` / `test123`

Группы и ключи доступа для студентов создаются в панели преподавателя на сайте.

### 2. Создание группы на сайте

Запустите сервер (шаг 4), войдите как преподаватель (`mr.dyadichev` / `test123`) и создайте группу в панели.

### 3. Загрузка лекций (опционально)

Положите PDF-файлы в `backend/uploads/lectures/`, затем:

```bash
python load_lectures.py --group-id 1
```

`--group-id` — ID группы, созданной на сайте.

### 4. Парсинг и загрузка вопросов
